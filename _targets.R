...
list(
  ...
  tar_target(diab_val_data, assessment(diab_val_split$splits[[1]])), 
  tar_target(diab_analysis_data, analysis(diab_val_split$splits[[1]])), 
  tar_target(diab_tt_split, initial_split(data = diab_analysis_data, prop = 0.82, strata = DIAGNOSIS)),  # ensures 70 / 15 / 15 overall split
  ...
  tar_target(naive_bayes_fit, naive_bayes_workflow %>% fit(data = diab_train)), 
  tar_target(naive_bayes_train_preds, predict(naive_bayes_fit, new_data = diab_train, type = "prob")),
  tar_target(naive_bayes_test_preds, predict(naive_bayes_fit, new_data = diab_test, type = "prob")),
  ...
  tar_target(knn_train_preds, predict(knn_fit, new_data = diab_train, type = "prob")),
  tar_target(knn_test_preds, predict(knn_fit, new_data = diab_test, type = "prob")), 
  ...
  tar_target(xgboost_tuning_res, tune_model(mod.workflow = xgboost_workflow, 
                                            cv.folds = diab_train_folds, 
                                            mod.type = "xgboost")), 
  tar_target(xgboost_best_params, select_best(xgboost_tuning_res, "roc_auc")),
  tar_target(xgboost_fit, fit_best_train(mod.workflow = xgboost_workflow,
                                         best.params = xgboost_best_params,
                                         train.dat = diab_train)),
  tar_target(xgboost_train_preds, predict(xgboost_fit, new_data = diab_train, type = "prob")),
  tar_target(xgboost_test_preds, predict(xgboost_fit, new_data = diab_test, type = "prob")), 
  tar_target(xgboost_var_imp, get_var_imp(mod.fit = xgboost_fit, mod.type = "xgboost")), 
  ...
  tar_target(nn_fit, fit_best_train(mod.workflow = nn_workflow,
                                    ...
                                    train.dat = diab_train)),
  tar_target(nn_train_preds, predict(nn_fit, new_data = diab_train, type = "prob")),
  tar_target(nn_test_preds, predict(nn_fit, new_data = diab_test, type = "prob"))
)
